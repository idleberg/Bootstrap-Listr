		...
		<body>
			<div class="container">
				...
				<? foreach($folder_list as $item) : ?>
						<tr>
							<td<? if (ENABLE_SORT) { ?> data-sort-value="<?=$item['lbname']?>"<? } ?>><? if (ENABLE_ICONS) { ?><i class="<?=$folder_icon?>">&nbsp;</i><? } ?><a href="<?=rawurlencode($item['bname'])?>/"><strong><?=utf8_encode($item['bname'])?></strong></a></td>
				...
				<? foreach($file_list as $item) : ?>
						<tr>
							<td<? if (ENABLE_SORT) { ?> data-sort-value="<?=$item['lname']?>"<? } ?>><? if (ENABLE_ICONS) { ?><i class="<?=$item['class']?>">&nbsp;</i><? } ?><a href="<?=rawurlencode($item['bname'])?>"><?=utf8_encode($item['bname'])?></a></td>
